...
function markdownTable(inputHTML : string) {
  let outputHTML: string[] = inputHTML.split("<br />");
  for (let i = 0; i < outputHTML.length; i++) {
    let line: string = outputHTML[i];
    if (line.includes(" | ")) {
      ...
      if (lineArray.length > 3) {
        ...
        for (let j = 0; j < lineArray.length; j++) {
          if (j < lineArray.length - 1 && lineArray[j + 1].includes("| ---")) {
            //heading row
            let tempLine: string[] = lineArray[j].split("|");
            tempLine.splice(0, 1);
            tempLine.splice(tempLine.length - 1, 1);
            let outLine: string = "<tr>";
